    ...
    <div class="min-h-screen relative overflow-hidden py-10 px-8 bg-waitt-dark">
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h2 class="text-4xl font-extrabold text-white mb-2">Presentations</h2>
                    <p class="text-lg text-gray-300">Manage presentation submissions and scheduling</p>
                </div>
                <a href="{{ route('moderator.presentations.create') }}" class="px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-xl text-lg shadow transition flex items-center gap-2">
            ...
            <div class="overflow-x-auto flex-1">
                <table class="min-w-full overflow-hidden shadow-2xl rounded-2xl bg-white/5 backdrop-blur-md">
                    <thead class="bg-white/10">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-200 uppercase tracking-wider">Title</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-200 uppercase tracking-wider">Speaker</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-200 uppercase tracking-wider">Time Slot</th>
                    ...
                    <tbody>
                        @forelse($presentations as $presentation)
                            <tr class="hover:bg-white/10 transition cursor-pointer">
                                <td class="px-6 py-4 whitespace-nowrap text-white font-semibold">{{ $presentation->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-200">{{ $presentation->speaker ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-200">{{ $presentation->time_slot ?? '-' }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap flex gap-4 items-center">
                                    <a href="{{ route('moderator.presentations.show', $presentation) }}" class="text-waitt-yellow hover:text-waitt-yellow" title="View"><svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                    <a href="{{ route('moderator.presentations.edit', $presentation) }}" class="text-waitt-yellow hover:text-waitt-yellow" title="Edit"><svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536M9 11l6 6M3 21h6l11-11a2.828 2.828 0 00-4-4L5 17v4z"/></svg></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-gray-300">There are currently no presentations.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
