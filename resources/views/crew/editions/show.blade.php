...
<x-crew-colorful-layout>
    ...
    <div class="py-5">
            <x-waitt.action-section>
                ...
                <x-slot name="content">
                    <x-waitt.details-list-item label="Edition title">
                        {{ $edition->name }}
                    </x-waitt.details-list-item>
                    <x-waitt.details-list-item label="Event Date & Time">
                ...
                <x-waitt.action-section>
                    ...
                    <x-slot name="content">
                        @forelse($edition->presentationTypes as $presentationType)
                            <div
                                class="border-transparent rounded-lg shadow-sm rounded-lg bg-slate-950/80 my-4">
                                    ...
                                    <dd>
                                        <div class="flex space-x-2">
                                            @can('update', $presentationType)
                                                <button class="border border-waitt-yellow text-waitt-yellow px-3 py-1 rounded-md hover:bg-gray-700 hover:cursor-pointer" :key="{{ $presentationType->id . '-edit' }}"
                                                ...
                                                >
                                                    <svg class="w-5 h-5 stroke-waitt-yellow" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23 23" fill="none" aria-hidden="true">
                                            ...
                                            @else
                                                <div class="tooltip">
                                                    <button disabled class="border border-gray-700 text-gray-700 px-3 py-1 rounded-md px-3 py-1 rounded-md">
                                                        <svg class="w-5 h-5 stroke-gray-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23 23" fill="none" aria-hidden="true">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                  d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125"/>
                                                        </svg>
                                                    </button>
                                            ...
                                            @can('delete', $presentationType)
                                                <button class="border border-red-700 text-red-700 px-3 py-1 rounded-md hover:bg-gray-700 hover:cursor-pointer" :key="{{ $presentationType->id . '-delete' }}"
                                                ...
                                                >
                                                    <svg class="w-5 h-5 stroke-red-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23 23" fill="none" aria-hidden="true">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            @else
                                                <div class="tooltip">
                                                    <button disabled class="border border-gray-700 text-gray-700 px-3 py-1 rounded-md px-3 py-1 rounded-md" :key="{{ $presentationType->id . '-delete' }}"
                                                        onclick="Livewire.dispatch('openModal', { component: 'presentation-type.delete-modal', arguments: { presentationTypeId: {{ $presentationType->id }} } })"
                                                    >
                                                        <svg class="w-5 h-5 stroke-gray-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23 23" fill="none" aria-hidden="true">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            @can('update', $edition)
                <x-waitt.action-section>
                    ...
                    <x-slot name="content">
                        @foreach($events as $event)
                            <div
                                class="border-transparent rounded-lg hover:cursor-pointer hover:bg-slate-950 shadow-sm rounded-lg my-4"
                                ...
                                <div class="px-4 py-6 flex justify-between">
                                    <dt class="text-sm font-medium leading-6 text-white">{{ $event->event->name }}</dt>
                ...
                @if($edition->id == optional(Edition::current())->id)
                    ...
                    <x-waitt.action-section>
                        ...
                        @if($edition->keynote_configured)
                            <x-slot name="content">
                                <x-waitt.details-list-item label="Keynote Speaker Name">
                                    {{ $edition->keynote_name }}
                                </x-waitt.details-list-item>
                                <x-waitt.details-list-item label="Keynote Speaker Description">
                                    {{ $edition->keynote_description }}
                                </x-waitt.details-list-item>
                                <x-waitt.details-list-item label="Keynote Speaker Photo">
