                    ...
                    <div class="py-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-2">
                            @forelse ($role->users as $user)
                                <div class="relative bg-waitt-dark border border-slate-950 shadow-sm rounded-lg p-2 flex items-center space-x-4">
                                    <div class="shrink-0">
                                        <img class="h-10 w-10 rounded-full" src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}">
                                    </div>
                                    <div class="flex-1">
                                        <div class="text-md font-medium text-gray-100">{{ $user->name }}</div>
                                        <div class="text-xs text-gray-400">{{ $user->email }}</div>
                                        @if($user->crew_team)
                                            <div class="text-xs text-gray-400">{{ 'Team: ' . ucfirst($user->crew_team) }}</div>
                                        @endif
                                    </div>
                                        ...
                                        <div class="absolute top-1 right-1 flex space-x-1">
                                            @if($role->name == 'event organizer')
                                                <button
                                                    ...
                                                    class="h-5 w-5 rounded-full bg-waitt-cyan-800 hover:bg-waitt-cyan-900 hover:cursor-pointer text-white flex items-center justify-center hover:bg-blue-700 focus:outline-none">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" class="h-3 w-3" viewBox="0 0 24 24"
                                            ...
                                            <button
                                                ...
                                                class="h-5 w-5 rounded-full bg-waitt-pink-800 hover:bg-waitt-pink-900 hover:cursor-pointer text-white flex items-center justify-center hover:bg-red-700 focus:outline-none">
                            ...
                            @can('invite-crew-member')
                                <button
                                    ...
                                    class="bg-waitt-dark border border-slate-950 h-15 text-waitt-yellow shadow-sm rounded-lg p-2 flex items-center space-x-2 hover:cursor-pointer hover:bg-slate-950 transition-all">
                                    <div class="shrink-0">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
