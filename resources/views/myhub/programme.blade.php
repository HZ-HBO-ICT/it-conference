            ...
            <div class="px-6 py-6 bg-gray-50 dark:bg-gray-800">
                <div class="flex flex-row justify-between">
                    ...
                    @if(Auth::user()->speaker)
                        <form method="POST" action="{{route('speakers.request.cancellation')}}">
                            @csrf
                            <div>
                                <button
                                   class="hidden lg:block md:block text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2">
                                    Do you want to cancel your presentation?
                                </button>
                            </div>
                        </form>
                    @endif
