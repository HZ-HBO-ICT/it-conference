<x-app-layout>
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('user.invitation.registration', $invitation) }}">
            @csrf

            <div class="mt-5">
                <x-label for="name" value="{{ __('Name') }}" class="after:content-['*'] after:text-red-500"/>
                <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="$invitation->name" required
