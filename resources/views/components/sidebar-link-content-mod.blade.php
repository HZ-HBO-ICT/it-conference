...
@php
    ...
    $bgColorClass = $isCurrentRoute ? 'bg-' . $roleColour . '-500 text-white': '';
@endphp
...
<li class="m-1">
       ...
       class="relative leading-6 font-semibold text-sm p-2 rounded-md gap-x-3 flex hover:bg-gray-200 dark:hover:bg-gray-600 hover:text-{{ $roleColour }}-400 dark:hover:text-{{ $roleColour }}-400 dark:text-white {{ $bgColorClass }}">
            ...
            @endif
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" aria-hidden="true" class="w-6 h-6 {{ $isCurrentRoute ? 'stroke-white' : 'stroke-' . $roleColour . '-400' }}">
