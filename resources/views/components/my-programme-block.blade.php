<form method="POST" action="{{route(
    (Auth::user()->presentations->contains($presentation) ? 'presentation-disenroll' : 'presentation-enroll')
    , $presentation)}}">
    @csrf
    <button class="hidden"></button>
    <div
        class="w-full rounded overflow-hidden shadow-lg {{$bgColor}} transition-all duration-300 transform hover:scale-105 hover:cursor-pointer"
