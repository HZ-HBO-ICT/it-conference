<x-content-moderator-layout>
    <div class="px-5 pt-5">
        <div id="breadcrumbs">
            <p class="text-gray-800 dark:text-gray-200"><span class="hover:text-crew-500"><a
                        href="{{route('moderator.schedule.overview')}}">Schedule management</a></span> /
                <span class="hover:text-crew-500"><a
                        href="{{route('rooms.index')}}">Available rooms</a></span> / Edit {{$room->name}}</p>
        </div>
        <h1 class="text-4xl font-extrabold text-gray-700 dark:text-white py-5">Edit {{$room->name}}</h1>

        <div class="pr-5">
            <form method="POST" action="{{route('rooms.update', $room)}}">
                @csrf
                @method('PUT')
                <div class="col-span-6 sm:col-span-4">
                    <x-label for="name" value="Room name"/>
                    <x-input id="name" name="name" type="text" value="{{$room->name}}" class="mt-1 block w-full"
                             disabled/>
                    <x-input-error for="name" class="mt-2"/>
                </div>
                <div class="col-span-6 sm:col-span-4 py-4">
                    <x-label for="max_participants" value="Maximum capacity of the room"/>
                    <x-input id="name" name="max_participants" type="number" value="{{$room->max_participants}}"
                             class="mt-1 block w-full focus:border-crew-500 dark:focus:border-crew-500 focus:ring-crew-500 dark:focus:ring-crew-500"/>
                    <x-input-error for="max_participants" class="mt-2"/>
                </div>
                <x-button
                    class="mt-5 dark:bg-crew-500 bg-crew-500 hover:bg-crew-600 hover:dark:bg-crew-600 active:bg-crew-600 active:dark:bg-crew-600">
                    Save
                </x-button>
            </form>
        </div>
    </div>
</x-content-moderator-layout>
